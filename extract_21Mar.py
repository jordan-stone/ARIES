...
class order:
    ...
    def extract(self,save_summary_fig_name=None,show=True):
        ...
        self.spectrum=(self.order[self.bot:self.top,:]).sum(axis=0)
        self.spatial=[]
    ...
    def extract_1st_and_2nd(self,save_summary_fig_name=None,show=True):
        self.profile_cent, self.profile_width=get_gauss(self.profile)
        ...
        a5.plot(self.second)
        a5.set_xlim(0,len(self.second))
        a5.set_ylim(np.mean(self.second)-0.5,np.mean(self.second)+0.5)
